...
def reconstructBase64String(chunk):
    ...
    #    pChunk = jchunk["data"]
    print
    pChunk["pic_id"], pChunk["pos"], pChunk["size"]
    ...
    if pictures.get(pChunk["pic_id"]) == None:
        pictures[pChunk["pic_id"]] = {"count": 0, "total": pChunk["size"],
                                      "pieces": [], "pic_id": pChunk["pic_id"]}
        print
        pictures

        pictures[pChunk["pic_id"]]["pieces"].insert(int(pChunk["pos"]),
                                                    pChunk["data"])
    else:
        pictures[pChunk["pic_id"]]["pieces"].insert(int(pChunk["pos"]),
                                                    pChunk["data"])
